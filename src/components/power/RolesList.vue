<template>
    <div>
                ...
                <el-table-column type="expand">
                    <template #default="{ row }">
                        <el-row :class="['bdbottom', index1 === 0 ? 'bdtop' : '', 'vcenter']"
                            ...
                            <!-- L1 -->
                            <el-col :span="5">
                                <el-tag closable @close="removeRightById(row, item1.id)" size="large">{{ item1.authName
                                    }}</el-tag>
                                <el-icon>
                                    <CaretRight />
                                </el-icon>
                                ...
                                <el-row :class="[index2 === 0 ? '' : 'bdtop', 'vcenter']"
                                    ...
                                    <el-col :span="6">
                                        <el-tag size="large" type="success" closable
                                            @close="removeRightById(row, item2.id)">{{ item2.authName }}</el-tag>
                                    </el-col>
                                    <el-col :span="18">
                ...
                <el-table-column width="300px" label="操作">
                    <template #default="{ row }">
                        <el-button type="primary" :icon="Edit" size="small"
                            @click="showEditDialog(row.id)">编辑</el-button>
                        <el-button type="danger" :icon="Delete" size="small"
                            @click="removeRoleById(row.id)">删除</el-button>
        ...
        <el-dialog v-model="setRightDialogVisible" title="分配权限" width="50%" @close="setRightDialogClosed">
            <el-tree style="max-width: 600px" :data="rightsList" :props="treeProps" show-checkbox default-expand-all
                node-key="id" :default-checked-keys="defKeys" ref="treeRef" />
            <template #footer>
                <div>
                    <el-button @click="setRightDialogVisible = false">取 消</el-button>
                    <el-button type="primary" @click="allocateRights">
                        确 定
                    </el-button>
                </div>
            </template>
...
const editRoleInfo = () => {
    editFormRef.value?.validate((valid) => {
        ...
        axios.put(`/roles/${editForm.value.roleId}`, {
...
const allocateRights = () => {
    ...
    axios.post(`/roles/${roleId.value}/rights`, {
        rids: defKeys.join(',')
    }).then(({ data: res }) => {
        if (res.meta.status !== 200) {
