        ...
        <vxe-column title="操作">
          <template #default="{ row }">
            <el-button
              ...
              >编辑</el-button
            >
            <el-button type="danger" size="large" :icon="Delete" @click="removeCateById(row.cat_id)"
              >删除</el-button
            >
          </template>
        </vxe-column>
...
const addCate = () => {
  addCateFormRef.value?.validate((valid) => {
    ...
    axios.post('categories', addCateForm).then(({ data: res }) => {
...
const editCate = () => {
  editCateFormRef.value?.validate((valid) => {
    ...
    axios
      .put(`categories/${editCateId.value}`, {
