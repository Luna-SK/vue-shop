<template>
    <div>
                    ...
                    <el-table :data="manyTableData" row-key="attr_id" order stripe>
                        ...
                        <el-table-column type="expand">
                            <template #default="{ row }">
                                <el-tag v-for="(item, index) in row.attr_vals" :key="index" type="primary" size="large"
                                    closable @close="handleClose(index, row)">{{ item }}</el-tag>
                                <el-input class="input-new-tag" v-if="row.inputVisible" ref="InputRef"
                                    v-model="row.inputValue" @keyup.enter="handleInputConfirm(row)"
        ...
        <el-dialog v-model="addDialogVisible" :title="'添加' + titleText" width="50%" @close="addDialogClosed">
        ...
        <el-dialog v-model="editDialogVisible" :title="'修改' + titleText" width="50%" @close="editDialogClosed">
...
const addParams = () => {
    addFormRef.value?.validate((valid) => {
        ...
        axios.post(`categories/${cateId.value}/attributes`, {
...
const showEditDialog = (id: number) => {
    axios.get(`categories/${cateId.value}/attributes/${id}`, {
        params: {
            attr_sel: activeName.value
        }
    }).then(({ data: res }) => {
...
const editParams = () => {
    editFormRef.value?.validate((valid) => {
        ...
        axios.put(`categories/${cateId.value}/attributes/${editForm.value.attr_id}`, {
...
const removeParams = (id: number) => {
    ElMessageBox.confirm(
        ...
        axios.delete(`categories/${cateId.value}/attributes/${id}`)
            .then(({ data: res }) => {
                if (res.meta.status !== 200) {
...
const saveAttrVals = (row: any) => {
    axios.put(`categories/${cateId.value}/attributes/${row.attr_id}`, {
